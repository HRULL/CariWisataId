...
<head>
    ...
    <title>Manajemen Destinasi - CariWisataID</title>
...
<body class="bg-gray-100 text-gray-900 min-h-screen flex flex-col">
    ...
    <header class="bg-gray-900 shadow p-4 flex items-center justify-between">
        ...
        <nav class="flex items-center space-x-6">
            <a href="{{ url('/dashboard') }}" class="text-white hover:text-blue-600 font-medium transition">Dashboard User</a>
            <a href="{{ route('logout') }}"
               onclick="event.preventDefault(); document.getElementById('logout-form').submit();"
               class="text-red-500 hover:text-red-700 font-medium transition">Logout</a>
    ...
    <div class="flex flex-1">
        ...
        <main class="flex-1 p-8 overflow-auto">
            <h1 class="text-3xl font-bold mb-6">Manajemen Destinasi</h1>

            <a href="{{ route('destinasi.create') }}" class="mb-6 inline-block bg-blue-600 text-white px-5 py-2 rounded hover:bg-blue-700 transition">
                + Tambah Destinasi
            </a>
                ...
                <table class="min-w-full border border-gray-300">
                    ...
                    <tbody>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="border px-4 py-2">{{ $dest->nama }}</td>
                            <td class="border px-4 py-2">{{ $dest->lokasi }}</td>
                            <td class="border px-4 py-2">Rp {{ number_format($dest->harga) }}</td>
                            <td class="border px-4 py-2">
                                <img src="{{ asset('storage/destinasi/' . $dest->image) }}" alt="{{ $dest->nama }}" class="w-20 h-14 object-cover rounded">
                            </td>
                            <td class="border px-4 py-2 space-x-3">
                                <a href="{{ route('destinasi.edit', $dest->id) }}" class="text-blue-600 hover:underline">Edit</a>
                                <form action="{{ route('destinasi.destroy', $dest->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin mau hapus?')">
                                    @csrf
                                    @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="5" class="text-center py-4 text-gray-500">Belum ada data destinasi.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
